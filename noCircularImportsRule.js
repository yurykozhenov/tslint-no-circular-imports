...
var OPTION_IGNORE_TYPE_IMPORTS = true;
...
function walk(context, tc) {
    ...
    function visitImportOrExportDeclaration(node, tc) {
...
function isInterfaceOrTypeAliasImported(node, tc) {
    ...
    if (node.importClause.name !== undefined) {
        ...
        return node.importClause.namedBindings.elements.map(function (binding) {
            return checkImportKind(binding.name, tc);
        }).every(function (isTypeDeclaration) { return isTypeDeclaration === true; });
    }
    return false;
}
function checkImportKind(name, tc) {
